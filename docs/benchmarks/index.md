...
| **Salami compositional poisoning** — N individually-benign, collectively-harmful writes (arXiv:2608.01637), #37 | **save 100%** (1200/1200) [Wilson 95% 99.7, 100]; **assembly 100%** (300/300) [98.7, 100]; benign control **0%** [0, 1.3] | `cargo run --release -p mnemo-salami-poisoning-bench` | [`docs/benchmarks/salami-compositional-poisoning.md`](salami-compositional-poisoning.md) · [`bench/results/salami_poisoning.json`](../../bench/results/salami_poisoning.json) | **Measures a gap, does not defend one** — no per-write control rejects a benign slice; retrieval reassembles the whole. Lexical/offline embedder (semantic is future); harm is a structural oracle, not an LLM. Compositional subset of #37 only. |
