...
fn a_read_by_one_caller_does_not_authorise_an_erasure_by_another() {
    ...
    let alices_lease = store.issue("alice", forget_scope());

    assert_eq!(
        store
            .check(alices_lease.id, "mallory", LeaseScope::ForgetSubject)
            .expect_err("a lease is not bearer authority — it is bound to who earned it"),
        LeaseError::WrongAgent
    );
    store
        .check(alices_lease.id, "alice", LeaseScope::ForgetSubject)
...
fn an_expired_lease_is_rejected() {
    ...
    let lease = store.issue("alice", forget_scope());
    std::thread::sleep(Duration::from_millis(10));
    assert_eq!(
        store
            .check(lease.id, "alice", LeaseScope::ForgetSubject)
...
fn a_lease_without_the_forget_scope_is_rejected() {
    ...
    let store = LeaseStore::new(60);
    let scopeless = store.issue("alice", BTreeSet::new());
    assert_eq!(
        store
            .check(scopeless.id, "alice", LeaseScope::ForgetSubject)
...
fn an_invented_lease_token_is_rejected() {
    ...
    assert_eq!(
        store
            .check(uuid::Uuid::now_v7(), "alice", LeaseScope::ForgetSubject)
...
fn concurrent_callers_get_independent_leases() {
    ...
    let handles: Vec<_> = ["alice", "bob", "carol"]
        ...
        .map(|who| {
            ...
            std::thread::spawn(move || (who, store.issue(who, forget_scope()).id))
        })
        .collect();
    ...
    for (owner, id) in &issued {
        store
            .check(*id, owner, LeaseScope::ForgetSubject)
            .expect("each caller can spend its own lease");
        for (other, _) in &issued {
            if other != owner {
                assert!(
                    store.check(*id, other, LeaseScope::ForgetSubject).is_err(),
